...
foreach ($profile in $userProfiles) {
  $wefPath = Join-Path -Path $profile.FullName -ChildPath "AppData\Local\Microsoft\Office\16.0\Wef\$manifestFileName"
  if (Test-Path -LiteralPath $wefPath) {
    Remove-Item -LiteralPath $wefPath -Force
  }
}

$currentUserWefPath = Join-Path -Path $env:LOCALAPPDATA -ChildPath "Microsoft\Office\16.0\Wef\$manifestFileName"
...
Write-Output "DocuID add-in uninstall cleanup completed."
