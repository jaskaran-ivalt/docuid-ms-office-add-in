...
export class DocuIdApiService {
  ...
  private static getApiInstance(): AxiosInstance {
    if (!DocuIdApiService.instance) {
      DocuIdApiService.instance = axios.create({
      ...
      DocuIdApiService.instance.interceptors.request.use((config) => {
      ...
      DocuIdApiService.instance.interceptors.response.use(
        (response) => response,
        (error: AxiosError) => {
          if (error.response?.status === 401) {
            DocuIdApiService.apiLogger.warn('Unauthorized - clearing session');
            AuthService.logout();
    ...
    return DocuIdApiService.instance;
  }
  ...
  static async getDocuments(): Promise<DocuIdDocument[]> {
    ...
    try {
      ...
      const response =
        await DocuIdApiService.getApiInstance().get<ApiResponse<{ files: DocuIdDocument[] }>>(url);
      ...
      DocuIdApiService.apiLogger.error(`Error fetching ${host} documents`, error as Error);
      throw error;
    }
  }
  ...
  static async getWordDocuments(): Promise<DocuIdDocument[]> {
    return DocuIdApiService.getDocuments();
  }
  ...
  static async getDocumentAccess(documentId: number): Promise<DocumentAccess> {
    ...
    try {
      ...
      const response =
        await DocuIdApiService.getApiInstance().get<ApiResponse<{ document: DocumentAccess }>>(url);
      ...
      DocuIdApiService.apiLogger.error('Error getting document access', error as Error);
      throw error;
    }
  }
  ...
  static async downloadDocumentContent(accessUrl: string): Promise<Blob> {
    try {
      ...
      if (
        ...
        accessUrl === 'demo-ppt-investor-url' ||
        accessUrl === 'demo-document-content-url' // legacy fallback
      ) {
        DocuIdApiService.apiLogger.info(`Returning demo content for URL: ${accessUrl}`);
        return DocuIdApiService.createDemoContent(accessUrl);
      }
      // -----------------------
      ...
      if (
        ...
        /^https?:\/\/(dev\.|www\.)?docuid\.net/i.test(finalUrl)
      ) {
        try {
          ...
          if (matchDownload && matchDownload[1]) {
            finalUrl = DOCUMENT_ROUTES.DOWNLOAD(parseInt(matchDownload[1], 10));
            DocuIdApiService.apiLogger.info(`Rewrote absolute URL to: ${finalUrl}`);
          } else if (matchContent && matchContent[1]) {
            finalUrl = DOCUMENT_ROUTES.CONTENT(parseInt(matchContent[1], 10));
            DocuIdApiService.apiLogger.info(`Rewrote absolute URL to: ${finalUrl}`);
          } else {
            // Fallback: use relative path
            finalUrl = urlObj.pathname + urlObj.search;
            DocuIdApiService.apiLogger.info(`Rewrote absolute URL to relative path: ${finalUrl}`);
          }
        } catch (_e) {
      ...
      if (finalUrl.startsWith('/') || finalUrl.includes(window.location.host)) {
        const response = await DocuIdApiService.getApiInstance().get(finalUrl, {
          responseType: 'blob',
        });
      ...
      DocuIdApiService.apiLogger.error('Error downloading content', error as Error);
      throw error;
    }
  }
  ...
  private static createDemoContent(accessUrl: string): Blob {
    ...
    if (accessUrl === 'demo-excel-content-url') {
      ...
      text =
        ...
        '\n' +
        'What is DocuID?\n' +
        'DocuID integrates biometric authentication directly into Microsoft Office,\n' +
        'enabling secure, identity-verified document access at enterprise scale.\n' +
        '\n' +
        'Key Features\n' +
        '  - Biometric login via iVALT mobile app (fingerprint & face recognition)\n' +
        '  - One-tap access to Word, Excel and PowerPoint files — no passwords\n' +
        '  - Every open is logged with verified user identity and timestamp\n' +
        '  - Share documents with recipient-verified, granular permissions\n' +
        '\n' +
        'How It Works\n' +
        '  1. Open the DocuID add-in from the Office ribbon\n' +
        '  2. Enter your registered mobile number\n' +
        '  3. Approve the biometric prompt on the iVALT app\n' +
        '  4. Browse and open your secured documents instantly\n' +
        '\n' +
        'Supported Hosts\n' +
      ...
      text =
        'iVALT DocuID  |  Investor Overview\n' +
        '\n' +
        'The Problem\n' +
        'Traditional document security relies on static passwords and shareable links\n' +
        'that can be forwarded, stolen or forgotten — exposing sensitive data.\n' +
        '\n' +
        'Our Solution\n' +
        'DocuID binds every document open to a verified biometric identity.\n' +
        'Only the intended recipient — confirmed on their own device — can access the file.\n' +
        '\n' +
        'Market Opportunity\n' +
        '  - 1.2 billion+ Microsoft Office users globally\n' +
        '  - $42 billion document security market (2025 estimate)\n' +
        '  - 78% of data breaches involve compromised credentials (Verizon DBIR 2024)\n' +
        '\n' +
        'Business Model\n' +
        '  - SaaS subscription per seat / month\n' +
        '  - Enterprise site licences with SSO integration\n' +
        '  - API access tier for custom workflow integrations\n' +
        '\n' +
        'Traction\n' +
        '  - Active pilot deployments in legal, finance and healthcare sectors\n' +
        '  - Zero credential-based document breaches across all deployments\n' +
        '\n' +
        'Contact\n' +
  ...
  static async shareDocument(payload: any): Promise<ApiResponse<ShareApiResponse>> {
    ...
    try {
      ...
      const response = await DocuIdApiService.getApiInstance().post<ApiResponse<ShareApiResponse>>(
        url,
        shareRequest
      );
      return response.data;
    } catch (error) {
      DocuIdApiService.apiLogger.error('Error sharing document', error as Error);
      throw error;
    }
  }
}
