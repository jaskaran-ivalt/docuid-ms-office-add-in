...
const DownloadSheet: React.FC<DownloadSheetProps> = ({ isOpen, onClose }) => {
  ...
  const platform = PLATFORM_DATA[activeTab];
      ...
      <div className={`download-sheet ${isOpen ? "open" : ""}`} role="dialog" aria-modal="true">
        ...
        <div className="download-sheet-body">
          <div className="qr-container">
            <img
              src={QR_IMAGES[activeTab]}
              alt={`QR code for ${platform.store}`}
              className="qr-code"
            />
            <p className="qr-label">
              Scan this QR code with your phone camera
              <br />
              to open the {platform.store}
            </p>
            <a
              href={platform.url}
              target="_blank"
              rel="noopener noreferrer"
              className="qr-store-link"
            >
        ...
        <div className="download-sheet-footer">
          <p className="download-sheet-footer-text">
            You need the iVALT mobile app to complete biometric authentication
          </p>
        </div>
      </div>
    </>
