import { Dropdown, type IDropdownOption, PrimaryButton, TextField, Toggle } from '@fluentui/react';
...
const DebugPanel: React.FC<DebugPanelProps> = ({ isOpen, onClose }) => {
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    const matchesContext =
      !contextFilter || log.context.toLowerCase().includes(contextFilter.toLowerCase());
          ...
          filteredLogs
            ...
            .map((log, index) => (
              ...
              >
                ...
                <div style={{ wordWrap: 'break-word', marginBottom: '4px' }}>{log.message}</div>
                {log.data && (
                  <pre
                    style={{
