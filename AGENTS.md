...
| --------------------------------- | ----------------------------------------- |
| `bun run validate:word`           | manifests/manifest.xml (prod)             |
| `bun run validate:excel`          | manifests/manifest.xml (prod)             |
| `bun run validate:powerpoint`     | manifests/manifest.xml (prod)             |
...
├── manifest.xml                   # Production (addon.docuid.net) - Word + Excel + PowerPoint
├── manifest-production.xml        # Installer packaging variant
└── manifest-dev.xml               # Development (localhost:3000) - Word + Excel + PowerPoint
...
A single unified manifest supports all three Office hosts (Word, Excel, PowerPoint) in both environments:

- **Dev manifest** (`manifest-dev.xml`): Points to `https://localhost:3000` - used with `bun run dev:*`
- **Prod manifests** (`manifest.xml`, `manifest-production.xml`): Point to `https://addon.docuid.net` - used with `bun run start:*`
- The same GUID is shared across all hosts, so the add-in appears once in the Office Add-in catalog
- Runtime host detection via `OfficeHostService` determines which `IDocumentHandler` to use
...
| -------------- | ------------------------------- | ---------------------------- |
| Manifest       | `manifests/manifest-dev.xml`    | `manifests/manifest.xml`     |
...
- A single unified manifest (`manifest.xml` / `manifest-dev.xml`) supports Word, Excel, and PowerPoint via multiple `<Host>` entries
...
- Use `manifest-dev.xml` exclusively
