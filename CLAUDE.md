...
- `pnpm run dev-server` - Start HTTPS development server on port 3000
- `pnpm start` - Start development server and sideload add-in in Word
- `pnpm run build` - Production build
- `pnpm run build:dev` - Development build
- `pnpm run lint` - Run ESLint with Office Add-ins plugin
- `pnpm run lint:fix` - Auto-fix ESLint issues
- `pnpm run validate` - Validate manifest.xml
- `pnpm run prettier` - Format code with Prettier

### Office Add-in Specific Commands
- `pnpm run signin` - Sign in to M365 account for testing
- `pnpm run signout` - Sign out of M365 account
- `pnpm run stop` - Stop sideloaded add-in

### Testing Commands
- Manual testing is done through `pnpm start` which sideloads in Word
...
### Development Testing
- Use `pnpm start` to sideload in Word
