...
const MANIFESTS: { path: string; app: string; label: string }[] = [
...
function enableDebugging(manifest: string) {
  try {
    execSync(`office-addin-dev-settings debugging ${manifest}`, { cwd: ROOT, stdio: 'pipe' });
...
async function main() {
  ...
  // Enable debugging for all manifests
  console.log('🔧 Enabling debugging...');
  for (const m of MANIFESTS) {
    enableDebugging(m.path);
    console.log(`  ✅ Debugging enabled for ${m.label}`);
  }
  ...
  // Sideload all versions
  console.log('📎 Sideloading add-ins...');
  for (const m of MANIFESTS) {
    sideload(m.path, m.app);
  }

  console.log('\n═══════════════════════════════════');
  console.log('  All versions ready!');
